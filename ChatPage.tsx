...
import { GramGptLogo, DownloadIcon, SendIcon, PaperclipIcon, CloseIcon, LogoutIcon } from './components/IconComponents';
import { runGemini } from './services/gemini';
...
const ChatPage: React.FC<{user: User}> = ({ user }) => {
  ...
  const handleAPISubmit = async (promptToSend: string, imageToSend: File | null) => {
    ...
    try {
        const response: GenerateContentResponse = await runGemini(userParts, promptToSend);
      ...
      if (err instanceof Error) {
        ...
        } else if (err.message.includes('block') || err.message.includes('কোনো উত্তর পাওয়া যায়নি') || err.message.includes('প্রক্রিয়া করা যায়নি') || err.message.includes('API কী সেট করা নেই')) {
  ...
  const handleLogout = async () => {
    try {
      ...
      setError("লগআউট করার সময় একটি সমস্যা হয়েছে।");
    }
  };
    ...
    <div className="app-container">
      <header className="header">
        ...
        <button onClick={handleLogout} className="logout-button" aria-label="Logout">
          <span style={{ textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap', maxWidth: '150px' }}>{user.email}</span>
          <LogoutIcon />
        </button>
      </header>
